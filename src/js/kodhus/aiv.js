  ...
  const revealElement = (
    ...
    if (scroll_pos + window.innerHeight > elementTop + elementHeight) {
      ...
      element.style.transition =
        "all " + duration + "ms " + delay + "ms " + timingFunction;
      //setTimeout(() => {
  ...
  revealElements.forEach((elem) => {
    revealElementsCached.push({
      ...
      delay: elem.getAttribute("data-delay")
        ? parseInt(elem.getAttribute("data-delay"))
        : 0,
      elementHeight: elem.offsetHeight,
      elementTop: elem.offsetTop,
      onChildren: elem.getAttribute("data-aiv-children")
        ? elem.getAttribute("data-aiv-children")
        : false,
        ...
        elementObj.element
          ...
          .forEach((element, index) => {
            ...
            revealElement(
              ...
              scroll_pos,
              elementObj.duration,
              elementObj.timingFunction,
              elementObj.delay + index * elementObj.childrenDelay
            );
          });
      } else {
        elementObj.element.style.transform =
          "translateY(" + 100 + "px) scale(1)";
